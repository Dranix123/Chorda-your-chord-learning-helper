...
test("builds the finished chord-learning product", async () => {
  ...
  assert.match(pitchTraining, /Skip baseline/);
  assert.match(pitchTraining, /No limit/);
  assert.match(pitchTraining, />Next</);
  assert.match(pitchTraining, /Replay sound/);
  assert.match(pitchTraining, /Measure your comfortable vocal range/);
  assert.match(pitchTraining, /The first range test cannot be skipped/);
  assert.match(pitchTraining, /Retest vocal range/);
  assert.match(pitchTraining, /End recording/);
  assert.match(pitchTraining, /Record again/);
  assert.match(pitchTraining, /Use recording/);
  assert.match(pitchTraining, /Recordings can be repeated until you choose one to submit/);
  assert.doesNotMatch(pitchTraining, /Play reference tone/);
  assert.match(pitchTraining, /feedback after each answer/);
  assert.match(pitchTraining, /88-key expansion/);
  assert.match(pitchTraining, /88-key completion/);
  assert.match(pitchTraining, /incomplete keys are prioritised/);
  assert.match(pitchTraining, /Practice while waiting/);
  assert.match(pitchTraining, /Practice focus/);
  assert.match(pitchTraining, /practiceOptions = \[\.\.\.PITCH_NAMES\]/);
  ...
  assert.doesNotMatch(pitchTraining, /setTimeout\(advance,\s*850\)/);
